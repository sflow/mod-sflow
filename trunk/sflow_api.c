...
static uint32_t httpOpEncodingLength(SFLSampled_http *op) {
    ...
    uint32_t elemSiz = stringEncodingLength(&op->host);
    uint32_t elemSiz = stringEncodingLength(&op->referrer);
    uint32_t elemSiz = stringEncodingLength(&op->useragent);
    uint32_t elemSiz = stringEncodingLength(&op->authuser);
    uint32_t elemSiz = stringEncodingLength(&op->mimetype);
    elemSiz += 20; /* method, bytes, uS, status */
    return elemSiz;
}
...
int sfl_receiver_writeCountersSample(SFLReceiver *receiver, SFL_COUNTERS_SAMPLE_TYPE *cs)
{
    ...
    for(elem = cs->elements; elem != NULL; elem = elem->nxt) {
        ...
        case SFLCOUNTERS_HTTP:
            ...
            putNet32(receiver, elem->counterBlock.http.methd_connect_count);
